...
drop fips
rename new fips
save "`path_intermediate'pop_tmf_total_97_09_monthly_ages2", replace
*/

merge 1:1 fips year month using "`path_intermediate'mort_tmf_total_ages_97_10_monthly"
...
foreach var of local sex{
	foreach age of local age_bins{	
		gen mrate_`var'_`age' = 100000*mort_`var'_`age'/pop_`var'_`age'_
		gen lmrate_`var'_`age' = log(mrate_`var'_`age')
		bysort year month: egen pop_`var'_`age'_weights = total(pop_`var'_`age'_)
		replace pop_`var'_`age'_weights =  pop_`var'_`age'_/pop_`var'_`age'_weights
	}
}
...
**** save ****
local path_intermediate /Users/arvidviaene/Dropbox/Viaene_MortalityHospAdmBrazil/Data/Intermediate/
save "`path_intermediate'merged_monthly_ages_97_09", replace
